...
class Vectorialmult(tk.Frame):
    ...
    def create_screen(self):
        ...
        ttk.Label(title_frame, text="Vektoriális szorzat", style='Title.TLabel').grid(row=0, column=0)
    ...
    def show_rules(self):
        ...
        math_rules = ttk.LabelFrame(tab, text='Vektoriális szorzat')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'-Két vektor vektoriális szorzatán azt a z = u x v vektort értjük, amelynek az iránya merőleges az u és v vektorok síkjára.\n'
                       f'-A hossza egyenlő a két vektor által kifeszített paralelogramma területével.\n'
                       f'-A két vektor akkor és csakis akkor párhuzamos egymással, ha vektoriális szorzatuk a nullvektor.').grid(
    ...
    def vector_create(self,tab):
        ...
        # ha elore generalunk ertekeket, akkor toltsuk fel a vektorokat
    ...
    def show_diagram(self, tab, vector1, vector2):
        ...
        lim = max(vector2_mx.max(),vector1_mx.max())
        min_lim = min(vector2_mx.min(), vector1_mx.min())
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title('Vektoriális szorzat')
        ...
        # vektorialis szorzat kiszamolasa gomb
        ttk.Button(self.diagram_frame, text="Kiszámítás", width=25,
                   command=lambda: self.solver_controller.solver_handler('vectorial_mult',self.diagram_frame,ax, vector1_mx,vector2_mx)).grid(row=6, column=0)
