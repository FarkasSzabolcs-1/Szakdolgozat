...
class Vectorsum(tk.Frame):
    ...
    def create_screen(self):
        ...
        ttk.Label(title_frame, text="Vektorok összeadása", style='Title.TLabel').grid(row=0, column=0)
    ...
    def show_rules(self):
        ...
        math_rules = ttk.LabelFrame(tab, text='Vektorok összeadása')

        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'-Két vektor összeadása kommutatív és asszociatív művelet:').grid(
            row=0, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'u + v = v + u').grid(
            row=1, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'-Zéró hosszúságú vektort nullvektornak nevezünk.').grid(
            row=3, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'v + 0 = v').grid(
            row=4, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'-Egy v-vel azonos irányú és hosszúságú de ellentétes irányítású vektort ellentett vektornak nevezünk.').grid(
            row=5, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'v + (-v) = 0').grid(
            row=6, column=0, sticky='we')
        ttk.Label(math_rules, justify='left', wraplength=800,
                  text=f'-A vektorok megfelelő koordinátáit adjuk össze.').grid(
    ...
    def show_diagram(self, tab, vector1, vector2):
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title('Vektorok összeadása')
        ...
        ttk.Button(self.diagram_frame, text="Kiszámítás", width=25,
                   command=lambda: self.solver_controller.solver_handler('vector_sum',ax, vector1_mx,vector2_mx)).grid(row=6, column=0)
