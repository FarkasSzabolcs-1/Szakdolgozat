...
class Matrixsum(tk.Frame):
    def __init__(self, parent, controller,solver_handler):
        super().__init__(parent)
        self.screen_controller = controller
    ...
    def create_screen(self):
        ...
        ttk.Label(title_frame, text="Mátrixok összeadása", style='Title.TLabel').grid(row=0, column=0)
    ...
    def show_rules(self):
        ...
        ttk.Label(general_rules,style='TLabel',justify='left',text=f'-Példa adatok tab: Random generált értékek.').grid(row=0,column=0,sticky='we')
        ttk.Label(general_rules,style='TLabel',justify='left', text=f'-Saját adatok tab: Felhasználó által megadott értékek.').grid(row=1,column=0,sticky='we')
        ttk.Label(general_rules,style='TLabel',justify='left', text=f'-Üres mezőket a program nem fogad el.').grid(row=2,column=0,sticky='we')
        ttk.Label(general_rules,style='TLabel',justify='left', text=f'-Szöveget tartalmazó mezőket a program nem fogad el.').grid(row=3,column=0,sticky='we')


        math_rules=ttk.LabelFrame(tab,text='Mátrixok összeadása')
        ttk.Label(math_rules,style='TLabel',justify='left',wraplength=800, text=f'-Két mátrix akkor adható össze, ha típusaik megegyeznek. Az összegmátrix i-dik sorának j-dik eleme az összeadandó mátrixok i-dik sorában és j-dik oszlopában levő elemek összege.').grid(row=0,column=0,sticky='we')


        general_rules.grid(row=0, column=0,columnspan=2,sticky='we')

        math_rules.grid(row=1, column=0,sticky='we')
    ...
    def build_example_datas(self):
        ...
        ttk.Label(block1).grid(row=1, column=0, columnspan=2)
        ...
        ttk.Label(block2).grid(row=5, column=0, columnspan=2)
    ...
    def build_matrix(self, tab, matrix1_sor, matrix1_oszlop, matrix2_sor, matrix2_oszlop):
        ...
        #biztonsági ellenőrzés a generalas elott. Csak INT.
        try:
        ...
        #hiba uzenet kiirasa rossz ertekek eseten
        except Exception as e:
            if self.secondary_frame is not None:
                self.secondary_frame.destroy()
            ttk.Label(self.error_frame, text="Kérem adjon meg számokat értékként!", style="Error.TLabel").grid(row=0,                                                                                       column=0,sticky="ew")
            print(f"    \033[91m-> Sikertelen matrix generalas :{e}\033[0m")
            self.error_frame.grid(row=4, column=0, columnspan=2, sticky="ew")
            return False

        # a ket matrix oszlop es sorszamanak ellenorzese
        if ((matrix1_sor_ertek != matrix2_sor_ertek or matrix1_oszlop_ertek != matrix2_oszlop_ertek) or (
                matrix1_oszlop_ertek <= 0 or matrix2_oszlop_ertek <= 0 or matrix2_sor_ertek <= 0 or matrix1_sor_ertek <= 0)):
            if self.secondary_frame is not None:
                self.secondary_frame.destroy()
            ttk.Label(self.error_frame, text="Két mátrixot csak akkor lehet összeadni ha típusaik megegyeznek!",
        ...
        # megoldas gomb
        ttk.Button(self.secondary_frame, text="Mátrixok összeadása",
                   command=lambda: self.solver_controller.solver_handler('matrix_sum',self.result_frame, self.matrix1_entries,
                                                                self.matrix2_entries)).grid(row=5, column=0,
                                                                                            columnspan=2)
